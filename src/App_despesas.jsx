...
import { Home, MapPin, GraduationCap, Plane, Download, Plus, Trash2, Check, ChevronLeft, RefreshCw, CloudOff, Activity, Bell, AlertTriangle, Receipt } from "lucide-react";
...
const CENTRO_CUSTO = ["Brasilia", "Congressos/Simposios", "Despesas Internas Escritorio", "Estoque", "Reembolso - Despesas operacionais", "Reembolso - Viagens internacionais", "Reembolso - Viagens nacionais", "Viagens Internacionais", "Viagens Nacionais"];
const CATEGORIA = ["Alimentacao", "Aluguel de carro", "Combustivel", "Compra de Moedas", "Confraternizacao/aniversario", "Escritorio - AI", "Estacionamento", "Frete/Envio de material", "Hotel/Airbnb", "Aniversario medico", "Aniversario secretaria"];
const PAGAMENTO = ["Cartao corporativo", "Cartao proprio", "Dinheiro", "PIX", "Boleto", "Transferencia"];
const DESP_STATUS = ["Pendente", "Em analise", "Aprovado", "Reembolsado", "Recusado"];

function ExpenseForm({ onSave, saving }) {
  const [f, setF] = useState({ centro: "", categoria: "", valor: "", data: todayISO(),
    vendedora: "", pagamento: "", status: "Pendente", descricao: "" });
  const save = async () => {
    if (!f.centro || !f.categoria || !f.valor) { alert("Preencha ao menos Centro de custo, Categoria e Valor."); return; }
    const ok = await onSave({
      centro: f.centro, categoria: f.categoria, valor: f.valor, data: f.data || null,
      vendedora: f.vendedora, pagamento: f.pagamento, status: f.status, descricao: f.descricao,
    });
    if (ok) setF({ centro: f.centro, categoria: "", valor: "", data: todayISO(), vendedora: f.vendedora, pagamento: "", status: "Pendente", descricao: "" });
  };
  return (
    <div>
      <Field label="Centro de custo"><Select value={f.centro} onChange={(e)=>setF({...f,centro:e.target.value})} options={CENTRO_CUSTO} /></Field>
      <Field label="Categoria"><Select value={f.categoria} onChange={(e)=>setF({...f,categoria:e.target.value})} options={CATEGORIA} /></Field>
      <Field label="Valor (R$)"><TextInput type="number" inputMode="decimal" value={f.valor} onChange={(e)=>setF({...f,valor:e.target.value})} placeholder="0,00" /></Field>
      <Field label="Data"><TextInput type="date" value={f.data} onChange={(e)=>setF({...f,data:e.target.value})} /></Field>
      <Field label="Quem lancou"><Select value={f.vendedora} onChange={(e)=>setF({...f,vendedora:e.target.value})} options={[...VENDEDORES, "Socio", "Equipe"]} /></Field>
      <Field label="Forma de pagamento"><Select value={f.pagamento} onChange={(e)=>setF({...f,pagamento:e.target.value})} options={PAGAMENTO} /></Field>
      <Field label="Status"><Select value={f.status} onChange={(e)=>setF({...f,status:e.target.value})} options={DESP_STATUS} /></Field>
      <Field label="Descricao"><TextInput value={f.descricao} onChange={(e)=>setF({...f,descricao:e.target.value})} placeholder="Detalhe da despesa" /></Field>
      <PrimaryBtn onClick={save} disabled={saving}><Plus size={18}/> {saving ? "Salvando..." : "Salvar despesa"}</PrimaryBtn>
    </div>
  );
}
...
const STATUS_BI = ["Autorizado", "Analise"];
const REALIZ = ["Realizada", "Nao realizada", "Reagendada", "Cancelada"];
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, padding: 14, marginBottom: 10 }}>
      ...
      <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 6 }}>
        ...
        {c.realizada && <span style={{ fontSize: 11, fontWeight: 700, color: "#fff", background: {Realizada:"#117864","Nao realizada":C.danger,Reagendada:C.amber,Cancelada:C.mute}[c.realizada]||C.mute, borderRadius: 999, padding: "3px 9px" }}>{c.realizada}</span>}
      ...
      {editing && (
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${C.line}` }}>
          ...
          {(realizada==="Realizada" || realizada==="Reagendada") && (
            <Field label={realizada==="Reagendada" ? "Nova data da cirurgia" : "Data da cirurgia"}>
...
export default function App() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      const [v, c, t, cir, d] = await Promise.all([db.list("visitas"), db.list("cursos"), db.list("viagens"), supaFetch("cirurgias?select=*&order=data_cotacao.desc"), db.list("despesas")]);
      setVisits(v || []); setCourses(c || []); setTravels(t || []); setCirurgias(cir || []); setDespesas(d || []);
    } catch (e) {
      setErro("conexao");
  ...
  const alertas = useMemo(() => {
    const hoje = todayISO();
    const lista = [];
    // Cirurgias autorizadas sem realizacao ha mais de 15 dias
    cirurgias.forEach((c) => {
      if (c.status_orig === "Autorizado" && c.realizada !== "Realizada" && c.data_auto) {
        const dias = Math.round((new Date(hoje) - new Date(String(c.data_auto).slice(0,10))) / 86400000);
        if (dias >= 15) lista.push({ tipo: "cir", grave: dias >= 30,
          texto: `${c.paciente || "Cirurgia"} (${c.terapia}) autorizada há ${dias} dias sem realização`, sub: `${c.medico} · ${c.cliente}` });
      }
    });
    // Follow-ups vencidos
    visits.forEach((v) => {
      if (v.prox_follow && String(v.prox_follow).slice(0,10) < hoje &&
          (v.follow_status === "Pendente" || v.follow_status === "Agendado")) {
        const dias = Math.round((new Date(hoje) - new Date(String(v.prox_follow).slice(0,10))) / 86400000);
        lista.push({ tipo: "fup", grave: dias >= 7,
          texto: `Follow-up vencido há ${dias} dia(s): ${v.cirurgiao}`, sub: `${v.cliente || ""} · ${v.vendedora}` });
      }
    });
    return lista;
  }, [cirurgias, visits]);
  ...
  const recentes = useMemo(() => {
    const marca = (arr, tipo) => arr.slice(0, 5).map((r) => ({ ...r, _tipo: tipo, _ts: r.created_at }));
    const todos = [...marca(visits, "Visita"), ...marca(courses, "Curso"), ...marca(travels, "Viagem")];
    return todos.sort((a,b) => String(b._ts).localeCompare(String(a._ts))).slice(0, 6);
  ...
  const exportDespesas = () => downloadCSV("despesas.csv",
    [{key:"data",label:"Data"},{key:"centro",label:"Centro de Custo"},{key:"categoria",label:"Categoria"},
     {key:"valor",label:"Valor"},{key:"vendedora",label:"Quem Lancou"},{key:"pagamento",label:"Pagamento"},
     {key:"status",label:"Status"},{key:"descricao",label:"Descricao"}],
    despesas.map(d=>({...d, data:brDate(d.data)})));
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.paper, fontFamily: "'Inter', system-ui, sans-serif", color: C.ink }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontFamily: "'Fraunces', serif", fontSize: 22, fontWeight: 600, lineHeight: 1.1 }}>
              {tab==="home"?"Lancamentos do campo":tab==="cirurgias"?"Cirurgias":tab==="visitas"?"Nova visita":tab==="cursos"?"Cursos & treinamentos":tab==="despesas"?"Despesas & reembolsos":"Agenda de viagens"}
            </div>
          </div>
          {tab==="home" && SUPA_OK && (
      ...
      <main style={{ padding: "18px 16px 96px", maxWidth: 560, margin: "0 auto" }}>
          ...
          <div>
            ...
            {!loading && alertas.length > 0 && (
              <div style={{ marginTop: 22 }}>
                <SectionTitle>Alertas ({alertas.length})</SectionTitle>
                {alertas.map((a, i) => (
                  <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", background: a.grave ? "#FBEAEA" : "#FBF3E6",
                    border: `1px solid ${a.grave ? C.danger : C.amber}`, borderRadius: 12, padding: "12px 14px", marginBottom: 8 }}>
                    <AlertTriangle size={18} color={a.grave ? C.danger : C.amber} style={{ flexShrink: 0, marginTop: 1 }} />
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: C.ink, lineHeight: 1.35 }}>{a.texto}</div>
                      {a.sub && <div style={{ fontSize: 12, color: C.mute, marginTop: 2 }}>{a.sub}</div>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {!loading && alertas.length === 0 && (
              <div style={{ marginTop: 22, display: "flex", gap: 8, alignItems: "center", background: "#EAF3EE",
                border: `1px solid ${C.sage}`, borderRadius: 12, padding: "12px 14px", color: C.pineDk, fontSize: 14, fontWeight: 600 }}>
                <Bell size={17} /> Nenhum alerta no momento. Tudo em dia!
              </div>
            )}
            ...
            {!loading && recentes.length > 0 && (
              <div style={{ marginTop: 22 }}>
                ...
                {recentes.map((r, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", gap: 8, background: C.card,
                    border: `1px solid ${C.line}`, borderRadius: 12, padding: "11px 14px", marginBottom: 8 }}>
                    <div>
                      <div style={{ fontSize: 14, fontWeight: 600, color: C.ink }}>
                        {r._tipo === "Visita" ? (r.cirurgiao || "Visita") : r._tipo === "Curso" ? (r.convidado || "Curso") : (r.evento || "Viagem")}
                      </div>
                      <div style={{ fontSize: 12, color: C.mute, marginTop: 2 }}>
                        {r._tipo === "Visita" ? `${r.vendedora || ""} · ${r.cliente || ""}` : r._tipo === "Curso" ? `${r.curso || ""}` : `${r.viajante || ""} · ${r.destino || ""}`}
                      </div>
                    </div>
                    <span style={{ fontSize: 11, fontWeight: 700, color: C.pineDk, background: C.sand, borderRadius: 999, padding: "3px 9px", height: "fit-content", whiteSpace: "nowrap" }}>{r._tipo}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{ marginTop: 22 }}>
              ...
              <ExportRow label="Despesas" count={despesas.length} onClick={exportDespesas} />
            </div>
        ...
        {tab === "despesas" && (
          <>
            <ExpenseForm saving={saving} onSave={(row)=>salvar("despesas", setDespesas, row)} />
            <div style={{ marginTop: 26 }}>
              <SectionTitle>Despesas ({despesas.length})</SectionTitle>
              {loading && <Empty text="Carregando..." />}
              {!loading && despesas.length===0 && <Empty text="Nenhuma despesa ainda. Preencha acima e salve." />}
              {despesas.map((d)=>(
                <RecordCard key={d.id} title={`R$ ${d.valor} - ${d.categoria}`}
                  subtitle={`${d.centro} - ${d.vendedora||""} - ${brDate(d.data)}`}
                  tags={[d.status, d.pagamento, d.descricao]} onDelete={()=>apagar("despesas", setDespesas, d.id)} />
              ))}
            </div>
          </>
        )}
        ...
        {NAV.map(({ id, label, Icon }) => {
          ...
          return (
            <button key={id} onClick={()=>setTab(id)} style={{ background: "none", border: "none", cursor: "pointer",
              display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "4px 4px",
